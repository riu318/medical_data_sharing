...
function FillStatesDropDown(oCountrySel, oStateSel) {
	...
	$("#"+oStateSel).append($("<option />",{
					value: '',
					text: 'Select State'
				}));

	country_val_selectedindex = document.getElementById(oCountrySel).selectedIndex;

    if (country_val_selectedindex >= 0) {
//        var region = oCountrySel.options[country_val_selectedindex].text;
        var region = $("#"+oCountrySel).val();
...
function FillCityDropDown(oStateSel, oCitySel) {
    ...
    //$('<option />', { value: '', text: '--- Select/Specify State ---' }).appendTo('#' + oStateSel);
	$("#"+oCitySel).append($("<option>",{
				value: region,
				text: region
			}));

	state_val_selectedindex = document.getElementById(oStateSel).selectedIndex;

    if (state_val_selectedindex >= 0) {
//        var region = oCountrySel.options[country_val_selectedindex].text;
        var region = $("#"+oCitySel).val();
        if (city_states[region]) {
            stateArr = city_states[region].split('|');
